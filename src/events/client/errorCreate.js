...
module.exports = (client, err, command, interaction) => {
    ...
    let embed = new Discord.EmbedBuilder()
        .setTitle(`🚨・${password}`)
        .addFields(
            { name: `${client.emotes.normal.check} ┇Guild`, value: `**> ${interaction.guild.name} (${interaction.guild.id})**`},
            { name: `${client.emotes.normal.tv} ┇Command`, value: `${command}`},
            { name: `💬 ┇Error`, value: `\`\`\`${err}\`\`\``},
            { name: `${client.emotes.normal.paper} ┇Stack error`, value: `\`\`\`${err.stack.substr(0, 1018)}\`\`\``},
        )
